...
W celu sprawdzenia poprawności testów wykorzystano szkielet programistyczny Jest z menadżera pakietów npm. Szkielet ten pozwala na łatwe skonfigurowanie testów oraz na bezproblemowe uruchomienie ich. 
...
Na listingu 12.2 przedstawiono test jednostkowy który sprawdza poprawność działania dodania użytkownika z wymaganymi przez bazę danych polami. Jeżeli któreś z tych pól jest puste, w takim przypadku test jest oblany. 
...
\begin{lstlisting}[style=javascript, caption = {Test jednostkowy utworzenia instancji modelu User}, label = {lst:provider}]
 test('should create user with required fields', async () => {
      ...
      // Haslo powinno być hashowane
      expect(user.password).not.toBe('password123'); 
    });
\end{lstlisting}

\subsection{Testy integracyjne}
Na listingu 12.3 przedstawiono test integracjny, który sprawdza proces rejestracji, kiedy użytkownik poda adres email w złym formacie. W tym przypadku podczas rejestracji serwer powinien wysłać nam żądanie o kodzie 400, które oznacza, że żądanie jest źle zformułwane. 
...
\end{lstlisting}
\vspace{5cm}
Na rysunku 12.1 przedstawiono sktueczność wszystkich testów zawartych w projekcie. Sktueczność napisanych testów jest na poziomie $100\%$, co oznacza, że najważniejsze części systemu aukcyjnego działają poprawnie.
...
\begin{figure}[h!]
  ...
  \caption{Raport z uruchomienia testów systemu auckyjnego}
