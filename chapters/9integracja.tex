...
\begin{itemize}
  ...
  \item \texttt{-sASYNCIFY} - umożliwienie asynchronicznego wykonywania kodu, niezbędne dla WebGPU,
...
Komponent \texttt{WebGPUCanvasProvider} (listing \ref{lst:provider}), który implementuje wzorzec Provider z React Context API, działa jako pojedyncza instancja zarządzająca cyklem życia modułu WebAssembly oraz elementem canvas, na którym wykonywane jest renderowanie. Provider obejmuje całą aplikację, zapewniając dostęp do API komponentu renderującego z dowolnego miejsca w drzewie komponentów.
...
\subsubsection{Funkcje pomocnicze dostępu do API}

Dostęp do funkcjonalności komponentu renderującego z poziomu komponentów potomnych realizowany jest za pomocą dedykowanych funkcji pomocniczych React (ang. \textit{hooks}). Funkcja \texttt{useWebGPUCanvas()} stanowi publiczny interfejs zwracający API do kontroli komponentu renderującego, którego strukturę przedstawia listing \ref{lst:api}.
...
React Portal umożliwia renderowanie komponentu potomnego do węzła DOM znajdującego się poza hierarchią rodzica. W przypadku integracji WebGPU, współdzielony canvas jest renderowany przez Portal do aktualnie aktywnego \enquote{slotu} - kontenera DOM wskazanego przez komponent \texttt{WebGPUCanvas}, jak przedstawiono na listingu \ref{lst:portal}.
...
Inicjalizacja modułu WebAssembly odbywa się w sposób asynchroniczny podczas pierwszego stworzenia komponentu canvas. Proces ten obejmuje wstrzyknięcie skryptu \texttt{main.js}, konfigurację obiektu \texttt{Module} oraz obsługę stanów ładowania.

\subsubsection{Konfiguracja obiektu Module}

Przed załadowaniem skryptu Emscripten tworzy się i ustawia globalny obiekt \texttt{window.\hspace{0pt}Module}, który zawiera funkcje zwrotne obsługujące zdarzenia cyklu życia (listing \ref{lst:module}).
...
Skrypt \texttt{main.js} jest dynamicznie dodawany do dokumentu HTML, jak przedstawiono na listingu \ref{lst:script}. Mechanizm zapewnia, że skrypt jest ładowany tylko raz, niezależnie od liczby montowań komponentu.
...
Podczas fazy \texttt{preRun} konfigurowane są punkty montowania dla persystentnych danych, jak przedstawiono na listingu \ref{lst:idbfs}.
...
\begin{itemize}
  \item \texttt{change\_model(modelName)} - zmiana wyświetlanego modelu 3D, której wywołanie przedstawiono na listingu \ref{lst:changemodel},
...
Element HTML canvas jest przekazywany do modułu WebAssembly przez właściwość \texttt{Module.canvas}, jak przedstawiono na listingu \ref{lst:canvas}. Emscripten automatycznie używa tego elementu do utworzenia kontekstu WebGPU:
...
Komponent \texttt{WebGPUCanvas} (listing \ref{lst:usage}) stanowi publiczny interfejs do osadzania komponentu renderującego 3D w dowolnym miejscu aplikacji. Obsługuje responsywne wymiarowanie oraz automatyczną zmianę modelu.
